...
async function changePlaylistName(playlist_id, description, public, access_token) {  
  const data = {
  ...
  try {
    const response = await axios.put(`https://api.spotify.com/v1/playlists/${playlist_id}`, data, {
        headers: {
            'Authorization': `Bearer ${access_token}`,
        }
    });
